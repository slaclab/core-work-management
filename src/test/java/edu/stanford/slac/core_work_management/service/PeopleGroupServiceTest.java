...
public class PeopleGroupServiceTest {
    ...
    public void findPeopleTest() {
        ...
        List<String> distinguishedNames = this.ldapTemplate.search(
                query().where("objectClass").is("user").and(
                        query().where("mail").is("user2@slac.stanford.edu")
                ),
        ...
        filterObject.and(new EqualsFilter("objectClass", "user"));
//        filterObject.and(new EqualsFilter("objectCategory", "person"));
        filterObject.and(new EqualsFilter("mail", "user2@slac.stanford.edu"));
